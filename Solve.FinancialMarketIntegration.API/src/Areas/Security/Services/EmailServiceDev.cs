...
namespace Solve.FinancialMarketIntegration.API.Areas.Security.Services
{
    public class EmailServiceDev : IEmailService
    {
        ...
        public EmailServiceDev(IOptions<EmailSettings> emailSettings, IHostingEnvironment environment)
        {
            ...
            Dictionary<string, string> valoresReplace = null)
        {
            ...
            var destination = Path.Combine(Environment.ContentRootPath, mail.Subject + ".html");

            using (FileStream stream = new FileStream(destination, FileMode.Create))
            {
                var data = Encoding.UTF8.GetBytes(mail.Body);
